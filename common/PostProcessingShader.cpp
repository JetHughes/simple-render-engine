...
PostProcessingShader::PostProcessingShader(std::string vertexshaderName, std::string fragmentshaderName): Shader(vertexshaderName, fragmentshaderName){
    ...
    effectModeID = glGetUniformLocation(programID, "effectMode");
...
void PostProcessingShader::setShaderMode(float t){
    // if (effectModeID == -1){
    //   printf("cant find effectMode");
    // }
    glUniform1f(effectModeID, (float)(t));    
}
